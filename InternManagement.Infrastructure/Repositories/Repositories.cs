...
{
    public EfCertificateRepository(AppDbContext context) : base(context) { }

    public async Task<CertificateDetailDto?> GetDetailByIdAsync(int id, CancellationToken ct = default)
    {
        var certificate = await _dbSet.FirstOrDefaultAsync(c => c.CertificateId == id, ct);
        if (certificate is null) return null;

        var internName = await _context.Users.Where(u => u.UserId == certificate.InternId).Select(u => u.FullName).FirstOrDefaultAsync(ct);
        var programName = certificate.ProgramId.HasValue
            ? await _context.TrainingPrograms.Where(p => p.ProgramId == certificate.ProgramId).Select(p => p.ProgramName).FirstOrDefaultAsync(ct)
            : null;
        var issuedByName = await _context.Users.Where(u => u.UserId == certificate.IssuedBy).Select(u => u.FullName).FirstOrDefaultAsync(ct);

        return new CertificateDetailDto(
            certificate.CertificateId, certificate.InternId, internName ?? "",
            certificate.ProgramId, programName,
            certificate.CertificateName, certificate.Description,
            certificate.IssuedDate, certificate.CertificateUrl,
            certificate.IssuedBy, issuedByName ?? "",
            certificate.CreatedAt);
    }

    public async Task<IEnumerable<CertificateDto>> GetAllDtoAsync(PaginationRequest pagination, CertificateFilter? filter = null, CancellationToken ct = default)
    {
        var query = _dbSet.AsQueryable();
        ...
        return await query
            ...
            .Select(c => new CertificateDto(
                ...
                c.Description, c.IssuedDate, c.CertificateUrl,
                c.IssuedBy, c.CreatedAt))
            .ToListAsync(ct);
    }

    public async Task<int> CountAsync(CertificateFilter? filter = null, CancellationToken ct = default)
    {
        var query = _dbSet.AsQueryable();
